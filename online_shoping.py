    ...
    while True:
        ...
        if rule=='add':
            ...
            if report_entry=='customers':
                for i in range(len(shop.customerslist)):
                    print((shop.customerslist[i].id+','+shop.customerslist[i].name+','+shop.customerslist[i].balance+','
                          +{}+','+{}+"\n").format(len(shop.customerslist[i].saved),len(shop.customerslist[i].orderslist)))
                                ...
                                for l in range(len(shop.customerslist[i].orderslist[j].orders)):
                                    for t in range(len(shop.reps)):
                                        for s in range(len(shop.reps.goods)):
                                            if shop.reps.goods[s].id==shop.customerslist[i].orderslist[j].orders[l].id:
                                                if shop.customerslist[i].orderslist[j].orders[l].value<=shop.reps.goods[s].value:
                                                    shop.customerslist[i].submitorder(shop.customerslist[i].orderslist[j])
                                                    shop.reps[t].removegood(shop.reps.goods[s],shop.customerslist[i].orderslist[j].orders[l].value)
                                                    shop.customerslist[i].balance-=shop.customerslist[i].orderslist[j].orders[l].calculateprice
                                                    shop.income+=shop.customerslist[i].orderslist[j].orders[l].calculateprice
